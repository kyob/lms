...
function module_main() {
    ...
    if (!empty($_GET['consent']))
        $LMS->DB->Execute('UPDATE customers SET consentdate = ?NOW? WHERE id = ?', array($SESSION->id));
    ...
    $fields_changed = $LMS->DB->GetRow('SELECT id FROM up_info_changes WHERE customerid = ?', array($SESSION->id));
                ...
                if (preg_match('/phone([0-9]+)/', $changes['fieldname'], $matches)) {
                    ...
                    switch ($changes['fieldname']) {
                        case 'im':
                            ...
                            if ($changes['fieldvalue'])
                                $DB->Execute('INSERT INTO imessengers (customerid, uid, type) VALUES (?,?,?)', array($changes['customerid'], $changes['fieldvalue'], IM_GG));
                            break;
                        case 'yahoo':
                            $DB->Execute('DELETE FROM imessengers WHERE customerid = ? AND type = ?', array($changes['customerid'], IM_YAHOO));
                            if ($changes['fieldvalue'])
                                $DB->Execute('INSERT INTO imessengers (customerid, uid, type) VALUES (?,?,?)', array($changes['customerid'], $changes['fieldvalue'], IM_YAHOO));
                            break;
                        case 'skype':
                            $DB->Execute('DELETE FROM imessengers WHERE customerid = ? AND type = ?', array($changes['customerid'], IM_SKYPE));
                            if ($changes['fieldvalue'])
                                $DB->Execute('INSERT INTO imessengers (customerid, uid, type) VALUES (?,?,?)', array($changes['customerid'], $changes['fieldvalue'], IM_SKYPE));
                            break;
                        case 'name':
                        case 'lastname':
                        case 'address':
